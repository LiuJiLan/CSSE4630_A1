 ...
 */
class OwnershipAnalysis_bkp(cfg: ProgramCfg)(implicit declData: DeclarationData) extends FlowSensitiveAnalysis(true) {
  ...
  def eval(exp: AExpr, env: statelattice.Element): valuelattice.Element = {
    ...
    exp match {
      ...
      case AUnaryOp(DerefOp, subexp, loc) =>
        ...
        evalStack() // dereferenced result is a stack value
  ...
  def localTransfer(n: CfgNode, s: statelattice.Element): statelattice.Element = {
    import tip.lattices.OwnershipElement._
    import valuelattice._
    ...
    n match {
      case r: CfgStmtNode =>
        ...
        NoCalls.assertContainsNode(r.data) // interprocedural calls not supported (I am not implementing Part 5)
        NoRecords.assertContainsNode(r.data) // record and field operations are ignored as per specification
        ...
        r.data match {
          ...
          case AAssignStmt(id: AIdentifier, rhs, _) =>
            ...
            rhs match {
              ...
              case y: AIdentifier =>
                val o = eval(y, s)
                val yo = if (o == FlatEl(Owns)) FlatEl(DisOwns) else o
